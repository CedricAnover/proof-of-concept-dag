...
def _cb_func(node: Node, dep_results: dict[str, Result]) -> Any:
    res = f"{node.label}-stdout"
    # print(f"[node-{node.label}] {dep_results}")
    print(f"[node-{node.label}] Result Data: {res}-{random.randint(1, 500)}")
    return res
